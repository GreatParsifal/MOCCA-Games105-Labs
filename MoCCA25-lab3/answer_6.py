...
def end_effector_track_control(
    ...
    # 2) 末端点 PD：虚拟弹簧-阻尼器
    pos_err = target_end_effector_pos - current_end_effector_pos
    vel_err = target_end_effector_vel - current_end_effector_vel

    kp_ee = 120.0
    kd_ee = 35.0
    f_end = kp_ee * pos_err + kd_ee * vel_err

    # 3) 建立“某个 body 的父关节”
    parent_joint_of_body = [-1] * num_bodies
    for j in range(num_joints):
        parent_joint_of_body[bodyB[j]] = j

    # 4) 从末端 body 一路向根节点回溯，把末端力映射到每个祖先关节
    cur_body = end_effector_body
    while parent_joint_of_body[cur_body] != -1:
        j = parent_joint_of_body[cur_body]
        A = bodyA[j]
        B = bodyB[j]

        joint_pos = x[B] + R[B] @ joints[j].from_bodyB

        r = current_end_effector_pos - joint_pos

        joint_torques[j] += np.cross(r, f_end)

        joint_torques[j] -= 2.0 * (w[B] - w[A])
