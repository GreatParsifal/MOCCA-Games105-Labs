...
def integration(x:np.ndarray, R:np.ndarray, v:np.ndarray, w:np.ndarray, h:float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    ####### 你的回答 #######
    x_next = x + v * h
    delta_R = Rotation.from_rotvec(w * h)
    R_cur = Rotation.from_matrix(R)
    R_update = delta_R * R_cur
    R_next = R_update.as_matrix()
...
def forward_dynamics(
    ...
    ####### 你的回答 #######
    v_next = v + h * (inv_m * f)
    Iworld = np.einsum('bij,bjk,bkl->bil', R, I, R.transpose(0, 2, 1))
    inv_Iworld = np.einsum('bij,bjk,bkl->bil', R, inv_I, R.transpose(0, 2, 1))
    Iw = np.einsum('bij,bj->bi', Iworld, w)
    w_cross_Iw = np.cross(w, Iw)
    torque = tau - w_cross_Iw
    dw = np.einsum('bij,bj->bi', inv_Iworld, torque)
    w_next = w + h * dw
