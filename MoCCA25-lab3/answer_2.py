...
def ball_Jacobian(x:np.ndarray, R:np.ndarray, v:np.ndarray, w:np.ndarray, joints:List[Joint]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for j, joint in enumerate(joints):
        ...
        # ERP: try to eliminate position error over one step
        rhs[j] = ERP * (pB_pos - pA_pos)
        #rhs[j] = 0
    #######################
...
def forward_dynamics_with_constraints(
    ...
    ##### 你的代码 #####
    I_world = np.einsum('bij,bjk,bkl->bil', R, I, R.transpose(0, 2, 1))
    inv_I_world = np.einsum('bij,bjk,bkl->bil', R, inv_I, R.transpose(0, 2, 1))
    for i in range(num_bodies):
        ...
        inv_M[s, s] = np.eye(3) * inv_m[i]
        inv_M[t, t] = inv_I_world[i]
    ###################
    ...
    ##### 你的代码 #####
    v0, w0 = forward_dynamics(m, I, inv_m[:, None], inv_I, x, R, v, w, f, tau, h)
    q0 = np.hstack([v0, w0]).reshape(-1, 1)
    ###################    
    ...
    # ball joints: 3 rows each
    for j in range(num_joints):
        jnt = joints[j]
        a = jnt.bodyA if jnt.bodyA >= 0 else num_bodies + jnt.bodyA
        b = jnt.bodyB if jnt.bodyB >= 0 else num_bodies + jnt.bodyB

        J[row:row+3, a*6:a*6+6] = J_ball[j, :, :6]
        J[row:row+3, b*6:b*6+6] = J_ball[j, :, 6:12]
        rhs[row:row+3, 0] = rhs_ball[j]
        row += 3

    # hinge joints: 2 rows each
    if num_hinge_joints > 0:
        for j in range(num_hinge_joints):
            jnt = hinge_joints[j]
            a = jnt.bodyA if jnt.bodyA >= 0 else num_bodies + jnt.bodyA
            b = jnt.bodyB if jnt.bodyB >= 0 else num_bodies + jnt.bodyB

            J[row:row+2, a*6:a*6+6] = J_hinge[j, :, :6]
            J[row:row+2, b*6:b*6+6] = J_hinge[j, :, 6:12]
            rhs[row:row+2, 0] = rhs_hinge[j]
            row += 2
    ###################
    ...
    ##### 你的代码 #####
    q_next = q0 + h * (inv_M @ J.T @ lambd)
    q_next = q_next.reshape(num_bodies, 6)
