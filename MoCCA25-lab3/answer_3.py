...
def hinge_Jacobian(x:np.ndarray, R:np.ndarray, v:np.ndarray, w:np.ndarray, joints:List[Joint]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for j, joint in enumerate(joints):
        ...
        axis_world = R[a] @ joint.hinge_axis
        axis_world = axis_world / (np.linalg.norm(axis_world) + 1e-12)

        u, v_axis = orthonormal_basis(axis_world)
        ...
        # ERP on the relative angular velocity components orthogonal to hinge axis
        w_rel = w[a] - w[b]
        rhs[j, 0] = -ERP * np.dot(w_rel, u)
        rhs[j, 1] = -ERP * np.dot(w_rel, v_axis)
