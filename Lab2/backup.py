...
class CharacterController():
    ...
    # 根据未来几帧的目标朝向更新平滑后的目标朝向，降低输入抖动导致的频繁转向切换
    ...
    def _choose_spin_mode(self, yaw_err, yaw_vel):
        ...
        # 本作业控制坐标中，符号约定与动画命名方向相反，这里做一次统一翻转
        return 'spin_counter_clockwise' if sign < 0.0 else 'spin_clockwise'

    def _choose_turn_mode(self, yaw_err, yaw_vel):
        sign = self._resolve_turn_sign(yaw_err, yaw_vel)
        # 与 spin 保持一致，统一翻转左右映射
        return 'turn_left' if sign < 0.0 else 'turn_right'

    def _resolve_turn_sign(self, yaw_err, yaw_vel):
        sign = yaw_err
        # 接近 0 或接近 pi 时，几何符号容易抖动，优先使用输入角速度确定方向
    ...
    def _should_hold_turn(self, turn_mode, yaw_err):
        """
        转向锁仅在“仍朝目标方向逼近”时生效；一旦越过目标就立即解锁，
        避免连续同向转弯时在第二次输入发生过转。
        """
    ...
    def _desired_mode(self, desired_rot_list, desired_vel_list, desired_avel_list):
        ...
        # 如果当前正在转向且尚未对齐目标，则锁住左右方向，防止边界处反复横跳
        ...
        # 大角度偏差优先原地 spin：仅在几乎不前进时触发，避免边走边突然半圈转向
        # 反向（接近 180 度）时优先直接 spin，避免先触发两段 turn 才掉头。
        ...
        # 常规 spin：仅在几乎不前进时触发，避免边走边突然半圈转向
    ...
    def _edge_score(self, edge, mode):
        """
        给当前 node 的每条出边打分，分高者作为下一段 transition
        """
    ...
    def _pick_target_edge(self, desired_rot_list, desired_vel_list, desired_avel_list):
        """
        只在当前 node 的 edges 里选下一段 transition motion
        """
    ...
    def _start_transition(self, target_node, desired_rot_list):
        ...
        # turn/spin 动作开头常有起势帧，跳过少量头帧可减少 walk->turn 的“顿一下”。
        start_skip = 0
    ...
    def update_state(self,
                     ...
                     desired_avel_list):
        """
        根据 desired 状态，从当前 node 的出边中选择一个合适的 transition motion。
        """
        self._update_smoothed_target_yaw(desired_rot_list)

        # 空输入时冻结当前帧，避免程序启动后“先走一下再扭动”
        ...
        # 只从当前节点的 outgoing edges 中选下一个动作
        ...
        if self.in_transition:
            ...
            if will_finish:
                # transition 完成时对齐到当前已混合出的姿态，避免切换后一帧位置突变
                self.runtime = {
                ...
                # 推进两端 runtime，使用统一步进避免 clip 循环处跳变
