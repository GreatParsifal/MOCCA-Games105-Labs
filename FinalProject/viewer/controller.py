...
from .viewer_new import SimpleViewer
import math
import numpy as np
...
class KeyAndPad():
    def __init__(self, viewer) -> None:
        ...
        self.action_states = {
            "dance": {"requested": False, "held": False},
            "upper_dance": {"requested": False, "held": False},
            "kick": {"requested": False, "held": False},
            "backflip": {"requested": False, "held": False},
        }
    ...
    def request_action(self, action_name):
        action_state = self.action_states[action_name]
        if not action_state["held"]:
            action_state["requested"] = True
            action_state["held"] = True

    def release_action(self, action_name):
        self.action_states[action_name]["held"] = False

    def consume_action(self, action_name):
        requested = self.action_states[action_name]["requested"]
        self.action_states[action_name]["requested"] = False
        return requested
    ...
    def set_gamepad_map(self):
        ...
        # Some Windows gamepad drivers expose a non-UTF-8 device name.
        # Poll buttons directly so Panda3D does not need to create an
        # InputDeviceNode with the raw device name.
        self.gait_button = GamepadButton.rshoulder()
        self.jump_button = GamepadButton.face_b()
        self.left_trigger_axis = InputDevice.Axis.left_trigger
        self.left_trigger_threshold = 0.5
        self.left_trigger_down = False
        self.action_buttons = {
            "dance": GamepadButton.face_y(),
            "kick": GamepadButton.face_a(),
            "backflip": GamepadButton.face_x(),
        }
        self.gamepad_jump_pressed = False
        self.gamepad_action_pressed = {
            "dance": False,
            "upper_dance": False,
            "kick": False,
            "backflip": False,
    ...
    def update_gamepad(self, task):
        ...
        self.gait = int(self.device.findButton(self.gait_button).pressed)
        left_trigger_value = self.device.findAxis(self.left_trigger_axis).value
        self.left_trigger_down = bool(left_trigger_value > self.left_trigger_threshold)
        jump_pressed = bool(self.device.findButton(self.jump_button).pressed)
        if jump_pressed and not self.gamepad_jump_pressed:
            self.request_jump()
        elif not jump_pressed and self.gamepad_jump_pressed:
            self.release_jump()
        self.gamepad_jump_pressed = jump_pressed

        for action_name, button in self.action_buttons.items():
            action_pressed = bool(self.device.findButton(button).pressed)
            if action_name == "dance":
                upper_action_pressed = action_pressed and self.left_trigger_down
                dance_action_pressed = action_pressed and not self.left_trigger_down

                if upper_action_pressed and not self.gamepad_action_pressed["upper_dance"]:
                    self.request_action("upper_dance")
                elif not upper_action_pressed and self.gamepad_action_pressed["upper_dance"]:
                    self.release_action("upper_dance")
                self.gamepad_action_pressed["upper_dance"] = upper_action_pressed

                if dance_action_pressed and not self.gamepad_action_pressed["dance"]:
                    self.request_action("dance")
                elif not dance_action_pressed and self.gamepad_action_pressed["dance"]:
                    self.release_action("dance")
                self.gamepad_action_pressed["dance"] = dance_action_pressed
            else:
                if action_pressed and not self.gamepad_action_pressed[action_name]:
                    self.request_action(action_name)
                elif not action_pressed and self.gamepad_action_pressed[action_name]:
                    self.release_action(action_name)
                self.gamepad_action_pressed[action_name] = action_pressed
    ...
    def set_key_map(self):
        ...
        self.viewer.accept('space', self.request_jump)
        self.viewer.accept('space-up', self.release_jump)
        self.viewer.accept('1', self.request_action, ['dance'])
        self.viewer.accept('1-up', self.release_action, ['dance'])
        self.viewer.accept('4', self.request_action, ['upper_dance'])
        self.viewer.accept('4-up', self.release_action, ['upper_dance'])
        self.viewer.accept('2', self.request_action, ['kick'])
        self.viewer.accept('2-up', self.release_action, ['kick'])
        self.viewer.accept('3', self.request_action, ['backflip'])
        self.viewer.accept('3-up', self.release_action, ['backflip'])
        self.viewer.accept('lshift', self.key_input, ['gait', 1])
        self.viewer.accept('lshift-up', self.key_input, ['gait', 0])
        self.viewer.accept('rshift', self.key_input, ['gait', 1])
        self.viewer.accept('rshift-up', self.key_input, ['gait', 0])
...
class Controller:
    def __init__(self, viewer) -> None:
        ...
        self.rotation = np.array([0,0,0,1], dtype=np.float64)
        self.ctrl_position = np.array([-self.cameractrl.position[0], self.cameractrl.position[2], -self.cameractrl.position[1]], dtype=np.float64)
        self.ctrl_center = np.array([-self.cameractrl.center[0], self.cameractrl.center[2], -self.cameractrl.center[1]], dtype=np.float64)
        ...
        self.init_key_input()
        self.halflife = 0.27
        self.move_speed = np.array([1.75, 1.25, 1.5])
    @property
    def node(self):
        return self._node
    @property
    ...
    @property
    def jump_requested(self):
        return self.input_device.jump_requested

    def consume_jump(self):
        return self.input_device.consume_jump()

    def consume_action(self, action_name):
        return self.input_device.consume_action(action_name)
    ...
    def update_pos(self):
        ...
        camera_fwd = self.ctrl_center - self.ctrl_position
        ...
        for i in range(self.future_step):
            rot = LQuaternionf(*rotation_trajectory_render[i])
            rot *= self._root_rot_corr
            self.futures[i].set_pos(-position_trajectory[i][0], -position_trajectory[i][2], position_trajectory[i][1])
            self.futures[i].set_quat(rot)
